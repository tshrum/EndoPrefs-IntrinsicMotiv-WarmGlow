...
r <- filter(r, Finished == "TRUE")  # removing people who didn't complete survey
r <- filter(r, ResponseId != "R_tQdQTzAIRYHyEKZ")  # removing duplicate mturk id, first run had no treatment completed

d <- data.frame(rowID_P1 = 1:length(r$StartDate))  # creating new dataframe for usable data  

d$mturk <- r$Q2  # Checked for duplicated IDs, 1 present - removed instance
d$responseId_P1 <- r$ResponseId

# Exporting an ID key
d %>% select(mturk, responseId_P1, rowID_P1) %>%
  write_csv(file = "/Users/tshrum/Projects/Intrinsic-vs-Extrinsic/data/idKey.csv")
d$duration_P1 <- as.numeric(r$`Duration (in seconds)`)/60
d$startTime_P1 <- r$StartDate
d$NAcount_P1 <- rowSums(is.na(r[,35:63])) + rowSums(is.na(r[,68:88]))
...
IDvars <- select(d,id_environmentalistP1:id_humansAbusingEnviroP1)
...
pwConvoPEBsvars <- select(d,pw_watchReadP1:pw_startConvoP1) 
...
pwPEBsvars <- select(d,pw_watchReadP1:pw_usePlasticWaterP1, -pwConverseIndexP1)
...
ccVars <- select(d, cc_SeriousnessP1:cc_PersonalHarmP1)
...
#checking index consistency
# wgVars <- select(d,wg_goodP1:wg_positiveP1) 
...
intentConvoPEBsvars <- select(d,intent_watchReadP1:intent_startConvoP1) 
...
intentPEBsvars <- select(d,intent_watchReadP1:intent_usePlasticWaterP1, -intentConverseIndexP1) 
